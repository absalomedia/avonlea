...
class Clients extends Admin
{
  ...
  // --------------------------------------------------------------------
    ...
    function notes($client_id)
    {
      ...
      // new notes?  Update, move them on, and tell them its good
        if ($notes_submit) {
            \CI::Clients()->updateClient($client_id, array('client_notes'=>$notes));
  ...
  // --------------------------------------------------------------------
    ...
    function edit()
    {
        ...
        if ($this->validation->run() == false) {
            ...
            $data['row'] = \CI::Clients()->get_client_info($data['id']);
            ...
            \CI::Clients()->updateClient($clientInfo['id'], $clientInfo);
  ...
  // --------------------------------------------------------------------
    ...
    function delete($client_id)
    {
      // get number of invoices for when we ask if they are sure they want to remove this client
        $data['numInvoices'] = \CI::Clients()->countClientInvoices($client_id);
  ...
  // --------------------------------------------------------------------
    ...
    function delete_confirmed()
    {
        ...
        if (\CI::Clients()->deleteClient($client_id)) {

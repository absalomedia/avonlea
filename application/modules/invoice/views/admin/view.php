...
<?php echo form_open('invoices/notes/'.$row->id, ['id' => 'private_note_form']); ?>
...
?>
    ...
    <?php echo form_open('invoices/payment', ['id' => 'enterPayment', 'name' => 'enterPayment'], ['id' => $row->id]); ?>
...
<?php endif; ?>
  ...
  <p class="error" id="emailSuccess" style="display: none;"><?php echo $this->lang->line('invoice_email_quote_success'); ?></p>
    ...
    <?php echo form_open('invoices/email/'.$row->id.'/quote', ['id' => 'emailQuote', 'name' => 'emailQuote'], ['invoice_number' => $row->invoice_number, 'isAjax' => 'false']); ?>
    ...
    <?php echo form_open('invoices/email/'.$row->id, ['id' => 'emailInvoice', 'name' => 'emailInvoice'], ['invoice_number' => $row->invoice_number, 'isAjax' => 'false']); ?>
...
<div class="invoiceViewHold">
  <div id="companyDetails">
    ...
    <p>
        <?php if ($companyInfo->address1 != '') {
    echo $companyInfo->address1;
}?>
        <?php if ($companyInfo->address2 != '') {
    echo ', '.$companyInfo->address2;
}?>
        <?php if ($companyInfo->address1 != '' || $companyInfo->address2 != '') {
    echo '<br />';
}?>
        <?php if ($companyInfo->city != '') {
    echo $companyInfo->city;
}?>
        <?php if ($companyInfo->province != '') {
    echo ', '.$companyInfo->province;
}?>
        <?php if ($companyInfo->country != '') {
    echo ', '.$companyInfo->country;
}?>
        <?php if ($companyInfo->postal_code != '') {
    echo ' '.$companyInfo->postal_code;
}?>
        <?php if ($companyInfo->city != '' || $companyInfo->province != '' || $companyInfo->country != '' || $companyInfo->postal_code != '') {
    echo '<br />';
}?>
    ...
    <ul id="invoiceHistory">
        ...
        if ($paymentHistory->num_rows() === 0) {
          ...
          <li>
            ...
            ?> : <?php echo $this->settings_model->get_setting('currency_symbol').$row->amount_paid;

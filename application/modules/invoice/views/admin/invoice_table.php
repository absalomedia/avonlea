...
<?php
    ...
    <td><?php echo anchor('invoices/view/'.$row->id, \CI::Settings()->getSettings('currency_symbol').number_format($row->subtotal, 2, $this->config->item('currency_decimal'), '')); ?></td>
    <td>
    <?php
    if ($row->amount_paid >= ($row->subtotal + .01)) {
        // paid invoices
        echo anchor('invoices/view/'.$row->id, $this->lang->line('invoice_closed'), ['title' => 'invoice status']);
        ...
        echo anchor('invoices/view/'.$row->id, $this->lang->line('invoice_open'), ['title' => 'invoice status']);
    } else {
        // owing more then the overdue days amount
            // convert days due into a timestamp, and add the days payement is due in seconds
            $due_date = mysql_to_unix($row->dateIssued) + (\CI::Settings()->getSettings('days_payment_due') * 60 * 60 * 24);
        $line = "<span class='error'>".timespan($due_date, now()).' '.$this->lang->line('invoice_overdue').'</span>';
        echo anchor('invoices/view/'.$row->id, $line, ['title' => 'invoice status']);
    }
            ?>
            </td>
        </tr>
